...
function mapScaleQuestion(): Grade5Question {
  ...
  return { id: qid(`map-scale-${scale}-${mapCentimetres}`), signature: `map-scale-${scale}-${mapCentimetres}`, type: 'context', skillId: 'ratio-map-scale', icon: '🗺️', visualTitle: `Tỉ lệ 1 : ${vi(scale, 0)}`, visualLines: [`Trên bản đồ: ${mapCentimetres} cm`, 'Tính khoảng cách ngoài thực tế'], instruction: 'Khoảng cách thực tế là bao nhiêu ki-lô-mét?', answers: numericAnswers(realKilometres, Math.max(1, scale / 100_000), 0), correctAnswer: realKilometres, hintSteps: [`Khoảng cách thực theo xăng-ti-mét: ${mapCentimetres} × ${vi(scale, 0)}.`, `Đổi xăng-ti-mét sang ki-lô-mét bằng cách chia 100 000.`, `Kết quả là ${realKilometres} km.`], explanation: `${mapCentimetres} × ${vi(scale, 0)} : 100 000 = ${realKilometres} km.` };
}
...
function frequencyQuestion(): Grade5Question {
  ...
  return { id: qid(`frequency-${wins}-${total}`), signature: `frequency-${wins}-${total}`, type: 'context', skillId: 'relative-frequency', icon: '🎲', visualTitle: `${wins} lần xuất hiện trong ${total} lần`, visualLines: ['Viết tỉ số và rút gọn'], instruction: 'Tỉ số số lần xuất hiện là bao nhiêu?', answers: stringAnswers(correct, [`${total}/${wins}`, `${wins + 1}/${total}`, `${wins}/${total + 1}`]), correctAnswer: correct, hintSteps: [`Viết ${wins}/${total}.`, 'Rút gọn cả tử và mẫu.', `Kết quả là ${correct}.`], explanation: `Tỉ số là ${wins}/${total}${`${wins}/${total}` === correct ? '' : ` = ${correct}`}.` };
}
